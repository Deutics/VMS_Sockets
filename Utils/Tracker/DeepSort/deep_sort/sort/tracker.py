...
class Tracker:
    ...
    def update(self, detections, classes):
        ...
        for track_idx, detection_idx in matches:
            ...
            self.tracks[track_idx].is_missed = False
        ...
        active_targets = [t._track_id for t in self.tracks if t.is_confirmed()]
        features, targets = [], []
        for track in self.tracks:
            if not track.is_confirmed():
                continue
            features += track.features
            targets += [track._track_id for _ in track.features]
            track.features = []
    ...
    def _full_cost_metric(self, tracks, dets, track_indices, detection_indices):
        ...
        app_cost = self.metric.distance(
            ...
            np.array([tracks[i]._track_id for i in track_indices]),
        )
